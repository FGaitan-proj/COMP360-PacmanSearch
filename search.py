...
def uniformCostSearch(problem):
    ...
    while p_queue.isEmpty() == False:
        ...
        if node not in C:
            ...
            for (x,y,z) in problem.getSuccessors(node):
                actions= action+[y]
                p_queue.update((x,actions,z+cost),z+cost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    p_queue.push((problem.getStartState(),[],0),starth)
    C = []
    while p_queue.isEmpty() == False:
        (node,action,cost) = p_queue.pop()
        if problem.isGoalState(node):
        ...
        if node not in C:
            ...
            for (x,y,z) in problem.getSuccessors(node):
                allacts= action+[y]
                Hofnode = heuristic(x, problem)
                p_queue.update((x,allacts,z+cost),z+cost+Hofnode)
